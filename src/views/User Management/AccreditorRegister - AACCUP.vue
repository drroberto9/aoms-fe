<template>
  <div>
      ...
      <form @submit.prevent="submit">
        ...
        >
          <multiselect
            ...
            required
          ></multiselect>
          <!--          <b-form-select id="createUser-9" v-model="campusSelect">-->
    ...
    <b-modal id="app" v-model="existingUserPrompt">
      ...
      {{ concatName(user) }} already exist as an unregistered Accreditor. Do you
      want to register {{ concatName(user) }}?
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      let campus_id = null;
      if (this.campusSelect) campus_id = this.campusSelect.id;
      let region = this.region;
      let accreditor_status = "Registered";
      ...
      this.$store
        ...
        .then(result => {
          if (result.data.status === true) {
            this.$store.dispatch("showAccreditor").then(result => {
    ...
    changeAccreditorStatus() {
      this.$isLoading(true);
      let user_id = this.user.id;
      let status = "Registered";
      this.$store
        .dispatch("changeAccreditorStatus", { user_id, status })
        .then(result => {
          if (result.data.status === true) {
            this.$store.dispatch("showAccreditor").then(result => {
              this.users = result.data.users;
    ...
    select(item) {
      if (item != null) {
        ...
        this.$router.push("/AccreditorProfile_AACCUP");
      }
    }
  },
  async created() {
    ...
    await this.$store
      .dispatch("showAccreditor")
      .then(result => {
        this.users = result.data.users.filter(
          item => item.accreditor_status === "Registered"
        );
        this.totalRows = this.users.length;
      })
      .catch(err => {
