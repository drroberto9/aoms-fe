  ...
  <div>
    ...
    <md-card style="padding: 15px 20px;">
      <md-tabs :md-active-tab="tabSelect" md-elevation="1" class="md-primary">
        <!--Primary-->
        <md-tab id="assign-uploads-0" md-label="Required" style="padding: 0">
          <br />
          ...
          >
            ...
            <template #cell(actions)="row">
              <md-button
                ...
                title="Select"
                class="md-icon-button md-dense md-primary"
                @click="select(row.item.container, 'assign-uploads-0')"
                style="outline: 0"
          ...
          >
            ...
            <template #cell(actions)="row">
              <md-button
                ...
                title="Select"
                class="md-icon-button md-dense md-primary"
                @click="select(row.item.container, 'assign-uploads-1')"
                style="outline: 0"
                ><md-icon>forward</md-icon></md-button
              >
              <md-button
...
export default {
  ...
  methods: {
      ...
      this.$store
        ...
        .then(result => {
          if (result.data.status === true) {
            this.$store
              .dispatch("showContainer", this.office.id)
              .then(result => {
    ...
    select(item, tab) {
      ...
      this.$router.push("/AssignUploadedDocuments");
    },
    setTagEdit() {
  ...
  async created() {
    this.$isLoading(true);
    if (
      this.tabSelect !== "assign-uploads-0" &&
      this.tabSelect !== "assign-uploads-1"
    )
      this.$store.commit("saveTabSelect", "assign-uploads-0");
    await this.$store
      .dispatch("showContainer", this.office.id)
      .then(result => {
        this.documentList = result.data.documents;
