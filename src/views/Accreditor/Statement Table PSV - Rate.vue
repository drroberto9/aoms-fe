  ...
  <div id="create">
    ...
    <md-card style="padding: 15px 20px; margin-bottom: 10px;">
          ...
          <template #cell(rating)="row">
            ...
            <div v-else>
              <b-form-input v-model="row.item.item_score" disabled>{{
                (row.item.item_score = getMean(row.item.benchmark_statement_id))
              }}</b-form-input>
            </div>
          </template>
      ...
      <div class="md-layout">
        <div class="md-layout-item"></div>
        <div class="md-layout-item" style="max-width: max-content">
          <h5>Area Mean: {{ roundNum(areaMean) }}</h5>
        </div>
      </div>
      ...
      <br /><br /><br />
      <md-card style="margin: 0; padding: 15px 20px">
...
export default {
  ...
  methods: {
    ...
    setScore() {
      ...
      let parameter_id = this.statementList[0].parameter_id;
      let assigned_user_id = this.assigned_user_id;
      let items = this.items;
      let parameter_mean = this.areaMean;
      let best_practices = [];
      this.$isLoading(true);
      this.$store
        .dispatch("setScore", {
  ...
  async created() {
    ...
    if (this.coordinator === 1) {
      await this.$store
        .dispatch("showAccreditorInstrument", this.program.program_id)
        .then(result => {
