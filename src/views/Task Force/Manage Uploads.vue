  ...
  <div>
    ...
    <b-modal id="app" v-model="editDocument" hide-footer>
      ...
      <form @submit.prevent="edit">
        ...
        <!--        <b-form-group-->
        <!--          id="editDocument-group-2"-->
        <!--          label="*Folder Type:"-->
        <!--          label-for="editDocument-2"-->
        <!--        >-->
        <!--          <b-form-select id="editDocument-2" v-model="container_type" required>-->
        <!--            <b-form-select-option value="Primary"-->
        <!--              >Required</b-form-select-option-->
        <!--            >-->
        <!--            <b-form-select-option value="Supplementary"-->
        <!--              >Supplementary</b-form-select-option-->
        <!--            >-->
        <!--          </b-form-select>-->
        <!--        </b-form-group>-->
    ...
    <md-card style="padding: 15px 20px;">
      <md-tabs :md-active-tab="tabSelect" md-elevation="1" class="md-primary">
        <!--Primary-->
        <md-tab id="task-uploads-0" md-label="Required" style="padding: 0">
          <br />
          ...
          >
            ...
            <template #cell(actions)="row">
              <md-button
                ...
                title="Select"
                class="md-icon-button md-dense md-primary"
                @click="select(row.item.container, 'task-uploads-0')"
                style="outline: 0"
        ...
        <!--Supplementary-->
        <md-tab id="task-uploads-1" md-label="Supplemental" style="padding: 0">
          <br />
          <div class="md-layout" style="margin-bottom: 15px">
            <div class="md-layout-item">
              <h4>{{ office.office_name }}</h4>
          ...
          >
            ...
            <template #cell(actions)="row">
              <md-button
                ...
                title="Select"
                class="md-icon-button md-dense md-primary"
                @click="select(row.item.container, 'task-uploads-1')"
                style="outline: 0"
                ><md-icon>forward</md-icon></md-button
              >
              <md-button
...
export default {
  ...
  methods: {
    ...
    select(item, tab) {
      ...
      this.$store.commit("saveUploader", 1);
  ...
  async created() {
    if (
      this.tabSelect !== "task-uploads-0" &&
      this.tabSelect !== "task-uploads-1"
    )
      this.$store.commit("saveTabSelect", "task-uploads-0");
    this.officeList = this.offices.filter(
      data => data.role_id === 1 || data.role_id === 2 || data.role_id === 11
    );
