  ...
  <div id="create">
    ...
    <b-modal id="app" v-model="containerDialog" size="xl" hide-footer>
      ...
      <div class="md-layout">
        <div class="md-layout-item">
          <!--          <h5>Instruction:</h5>-->
          <h6>Areas suited for {{ instrument.area_name }} are:</h6>
          <ul style="color: red">
            <li v-for="area in instrumentTags" :key="area.id">
              {{ area.tag }}
            </li>
          </ul>
        </div>
          ...
          <div class="md-layout">
            ...
            >
              <b-form-input
                ...
                @input="searchDocumentTable(checkAttachment())"
                type="text"
              ></b-form-input>
            </div>
          </div>
    ...
    <md-card style="padding: 15px 20px; margin-bottom: 10px;">
      ...
      <div>
        ...
        >
              ...
              <div class="md-layout">
                ...
                >
                  <md-button
                    ...
                    @click="
                      statement = row.item;
                      filterDocumentArea = filterAreaFiles();
                      filterType = 'Primary';
                      search_container = '';
                      attachmentType = 0;
          ...
          <template #head(year_1)>
            <div class="md-layout">
              <div
                class="md-layout-item"
                style="max-width: 45px; padding-top: 10px"
              >
                Year:
              </div>
              <div class="md-layout-item">
                <b-form-select v-model="year_1">
                  <b-form-select-option :value="new Date().getFullYear()">{{
                    new Date().getFullYear()
                  }}</b-form-select-option>
                  <b-form-select-option :value="new Date().getFullYear() - 1">{{
                    new Date().getFullYear() - 1
                  }}</b-form-select-option>
                  <b-form-select-option :value="new Date().getFullYear() - 2">{{
                    new Date().getFullYear() - 2
                  }}</b-form-select-option>
                  <b-form-select-option :value="new Date().getFullYear() - 3">{{
                    new Date().getFullYear() - 3
                  }}</b-form-select-option>
                  <b-form-select-option :value="new Date().getFullYear() - 4">{{
                    new Date().getFullYear() - 4
                  }}</b-form-select-option>
                </b-form-select>
                <!--                <b-form-input v-model="year_1" type="number"></b-form-input>-->
              </div>
            </div>
          </template>
          ...
          <template #head(year_2)>
            <div class="md-layout">
              ...
              <div class="md-layout-item">
                ...
                <!--                <b-form-input v-model="year_2" type="number"></b-form-input>-->
              </div>
            </div>
          </template>
          ...
          <template #head(year_3)>
            <div class="md-layout">
              ...
              <div class="md-layout-item">
                ...
                <!--                <b-form-input v-model="year_3" type="number"></b-form-input>-->
              </div>
            </div>
          </template>
      ...
      <div
        v-if="instrument.area_name === 'LICENSURE EXAM'"
        class="md-layout"
      ...
      <!-- v-if="
            !statementList.find(
              x => x.parent_statement_id === row.item.benchmark_statement_id
            )
          " -->
...
export default {
  ...
  methods: {
    ...
    areaRemoveAttachment() {
      ...
      this.$store
        ...
        .then(result => {
          if (result.data.status === true) {
            let index = this.checkAttachment().findIndex(
              data =>
                data.id === this.fileSelected.instrument_program_document_id
            );
            this.checkAttachment().splice(index, 1);
            this.$isLoading(false);
            this.snackbarText = result.data.message;
            this.makeToast("Success!", "primary");
          } else {
      ...
      if (documents) {
        if (this.filterType === "Primary")
          return documents.filter(
            items =>
              items.container.type === "Primary" ||
              items.container.type === null
          );
    ...
    filterAreaFiles() {
      if (this.instrument.area_name === "INSTRUCTION") return 3;
      else if (this.instrument.area_name === "EXTENSION") return 4;
      else if (this.instrument.area_name === "RESEARCH") return 5;
      else if (this.instrument.area_name === "FACULTY") return 2;
      else if (this.instrument.area_name === "CONSORTIA OR LINKAGE") return 3;
      else if (this.instrument.area_name === "LICENSURE EXAM") return 3;
      else if (this.instrument.area_name === "LIBRARY") return 7;
      else return 0;
    },
    // filterAreaInstruction() {
    //   if (this.instrument.area_name === "INSTRUCTION")
    //     return "Areas I, II, III, IV, VII, VIII, IX and X";
    //   else if (this.instrument.area_name === "EXTENSION")
    //     return "Areas I, II, VI, V, VIII and X";
    //   else if (this.instrument.area_name === "RESEARCH")
    //     return "Areas I, II, VI, V, VIII and X";
    //   else if (this.instrument.area_name === "FACULTY")
    //     return "Areas II, III and X";
    //   else if (this.instrument.area_name === "CONSORTIA OR LINKAGE")
    //     return "Areas III, V, VI and X";
    //   else if (this.instrument.area_name === "LICENSURE EXAM")
    //     return "Areas III and IV";
    //   else if (this.instrument.area_name === "LIBRARY")
    //     return "Areas VII, VIII and X";
    // },
    ...
    checkAttachment() {
      if (this.attachmentType === 0) {
        return this.filterAttachments(this.statement.id);
  ...
  async created() {
    ...
    await this.$store
      .dispatch("showInstrumentTag", this.instrument.area_instrument_id)
      .then(result => {
        this.instrumentTags = result.data;
      })
      .catch(err => {
        this.snackbarText = err + ". Please contact the administrator";
        this.makeToast("Error", "danger");
