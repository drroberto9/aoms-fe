    ...
    <md-card style="padding: 15px 20px; margin-bottom: 10px;">
      <div class="md-layout">
        <div class="md-layout-item">
          <h4 v-if="campus">{{ campus.campus_name }}</h4>
...
export default {
  ...
  methods: {
    ...
    addStatement() {
      ...
      this.$store
        .dispatch("createCriteriaStatement", {
          statement,
          type,
          statement_parent,
          instrument_id
    ...
    async changeParameter(item) {
      ...
      await this.$store
        .dispatch("showStatement", item.id)
        .then(result => {
          this.$isLoading(false);
          this.statementList = searchByParameter(
            result.data,
  ...
  async created() {
    ...
    if (this.tabSelect === "manage-instruments-2") {
      await this.$store
        .dispatch("showInstrument", 42)
        .then(result => {
    ...
    } else if (this.tabSelect === "manage-instruments-3") {
      await this.$store
        .dispatch("showInstrument", 47)
        .then(result => {
          this.msiList = result.data.instrument;
        })
        .catch(err => {
